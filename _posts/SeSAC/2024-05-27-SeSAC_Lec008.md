...
위 코드에서는 부모의 포인터 배열을 만들어 0번과 1번에 자식을 동적할당하고 포인터를 이용해 함수를 호출하고 동적할당을 해제하는 작업을 진행합니다.
그런데 출력을 확인해보면 자식의 함수나 소멸자가 아닌 부모의 함수와 소멸자가 호출되는 것을 확인할 수 있습니다.

이는 <u>재정의된 함수는 호출하는 객체의 타입에 따라 호출되는 함수가 달라지기 때문</u>입니다.
