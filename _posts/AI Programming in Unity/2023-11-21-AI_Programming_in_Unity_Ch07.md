...
### 세트 확장
만약 두 번째 마법사인 `짐Jim`이 있다고 가정해봅시다. "짐의 '위기' 상태는 밥보다
낮은 20이다" 라고 했을 때 퍼지 로직의 장점이 나타나게 됩니다. 바로 에이전트가 변경되었더라도
시스템은 동일하다는 점입니다. 그렇기에 모든 에이전트는 각기 고유한 개성을 부여받아 제각기
다른 행동 패턴을 보일 수 있습니다.

또한 규칙이 꼭 3가지일 필요도 없습니다. 퍼지 로직 컨트롤러는 미리 정의한 규칙에 따라
동작하며 얼마나 많은 세트가 존재하는지 또는 얼마나 많은 가능성을 가질 수 있는지에 대한
시스템적인 제약을 하지 않습니다.

### 데이터 명확하게 하기
지금까지의 내용을 살펴봤을 때 바이너리 로직에 비해 퍼지 로직은 애매함을 추구하는 것 같습니다.
하지만 퍼지 로직은 `데이터`와 `의사결정`을 명확히 하기 위함입니다. 다음 연산을 살펴봅시다.
...
아마 해당 코드를 보면 바이너리 컨트롤러가 아닌가 싶을 수 있습니다. 하지만 잘 살펴보면
`if`문 내부에서 상태 구분을 수행하기 때문에 퍼지 로직의 개념이 적용된 것입니다. 그리고 마지막 단계에
이르러서는 컴퓨터의 처리 방식에 의존해야 합니다.

![image](https://github.com/GunnHB/gunnHB.github.io/assets/117302300/492f0a50-8ad4-4a97-ad1e-e1ccea586cf9){: width="70%" height="70%"}

- 단순 데이터로 시작합니다. 단순 데이터는 매우 구체적인 실제의 값을 의미합니다.
- 퍼지화 단계에서 에이전트의 의사결정에 필요한 추상적이고 모호한 데이터를 만듭니다.
- 추론 단계에서 에이전트는 데이터의 의미를 해석합니다. 에이전트는 주어진 규칙에 기반해
  상태를 판단합니다. 이때는 사람이 의사결정을 할 떄와 비슷한 과정이 관여됩니다.
- 역퍼지화 단계는 사람에게 친숙한 형태의 데이터를 컴퓨터가 이해할 수 있는 명확한 데이터로 다시 변환합니다.
- 다시 구체화된 데이터를 가지고 에이전트의 의사결정을 합니다.

## 최종 데이터 사용
퍼지 컨트롤러의 출력 데이터는 `행동 트리`나 `유한 상태 기계`를 사용해서 처리할 수 있습니다.
물론 다수 컨트롤러의 출력을 결합할 수도 있습니다. 사실 다양한 조합을 사용할수록 좀 더 높은 수준의
인공지능을 보여줄 가능성이 큽니다.

![image](https://github.com/GunnHB/gunnHB.github.io/assets/117302300/e8210504-66ce-4339-bc12-fb99cbada24b){: width="70%" height="70%"}

체력에 대한 요소는 앞에서도 살펴봤지만 그 외에도 고려해야할 요소들이 많은 것을 알 수 있습니다.
이는 마치 `트리`처럼 보이기도 합니다. 각 노드를 평가할 때 퍼지 로직을 사용하는 행동 트리를 만들어도
아무 문제가 되지 않습니다. 이 두 개념을 합치면 강력하고 섬세하게 동작하는 인공지능 시스템을 만들 수 있습니다.

## 요약
퍼지 로직은 일단 기본 개념을 잘 이해하고 나면 그리 혼란스럽지 않은 기술입니다.

바이너리 시스템으로는 하기 힘든 일을 어떻게 퍼지 로직으로 처리하는지도 살펴봤습니다.
또한 멤버 함수와 멤버십의 수준, 퍼지 세트의 개념을 사용한 자신만의 퍼지 로직 컨트롤러를 
구현하는 방법을 살펴봤습니다. 