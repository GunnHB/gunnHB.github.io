...
이러한 선언을 `using 선언using-declaration`이라고 합니다. using 선언한 이름은 다른 이름과 비슷하게 동작합니다.

중괄호 안에서 using 선언으로 정의한 이름은 그 지점부터 중괄호가 끝날 때까지만 유효합니다.
{: .notice--warning}

### while문 대신 for문 사용하기
이전에 작성했던 while문의 바깥 구조만 작성하면 다음과 같습니다.
...
while문은 조건에 따라 본문이 실행되고 본문은 r의 값을 변경하여 조건을 다시 비교합니다.
이러한 구조는 다음과 같이 더 간단하게 작성할 수 있습니다.
...
for문은 본문을 거칠 때마다 r은 0부터 rows - 1까지 연속된 값을 갖습니다. 0은 범위의 시작이고
rows는 범위의 `마지막보다 하나 큰 값off-the-end-value`입니다. 이러한 범위를 `반 개방 범위half-open range`라고 하며
'[시작 값, 마지막보다 하나 큰 값)'으로 표현합니다.

의도적으로 범위가 비대칭적인 것을 보여주려고 짝이 맞지 않는 괄호들인 [ )를 사용합니다.
{: .notice--warning}
...
첫 번째 행은 `for 헤더for header`입니다. for 헤더는 실행문으로 이루어진 `for 본문for body`를 제어합니다.

for 헤더의 '초기 실행문' 부분은 for문이 시작할 때 한 번만 실행합니다. '초기 실행문'에서 정의한 변수는 for문
밖에서 소멸하므로 for문 이후의 코드에서는 접근할 수 없습니다.

프로그램은 for문을 반복 실행하면서 '조건'도 반복해서 판별합니다. 조건이 참을 반환하면 for문의 본문을 실행합니다. 
그 이후에 헤더의 '표현식'을 실행합니다. 그런 다음 조건 판별을 반복하면서 조건이 거짓을 반환할 때까지 본문과 헤더의
'표현식'을 계속해서 실행합니다.
...
'초기 실행문'이나 '실행문'은 자체에 세미콜론이 있으으로 표기를 하지 않았습니다.
{: .notice--warning}

### 중복 연산 줄이기
[[하나 이상의 문자를 출력]](#행-출력하기)이라는 주석 부분을 구현한 코드는 각각

- 별표
- 공백
- 인사말

세 부분으로 나눌 수 있습니다. 예제 프로그램에서는 별표나 공백 하나를 출력하고 불변성 유지를 위해
c 값을 바꿉니다. 이는 **<u>둘 이상의 같은 실행문</u>**을 하나로 합해 프로그램을 바꿀 때를 고려하는 것입니다.

인사말을 출력하는 부분을 첫 번째 순서로 둔다면 다른 두 부분은 불변성을 유지할 수 있도록 
c 값을 증가시키는 코드를 하나로 줄일 수 있습니다.
...
## 카운팅
프로그래머는 숫자를 셀 때 1이 아닌 0부터 세는 습관이 있습니다.
...
0부터 셀 때는 비교 연산자로 !=를 사용하고 1부터 셀 떄는 비교 연산자로 <=를 사용합니다.
둘 모두 for문을 반복하는 횟수는 같습니다만 왜 0부터 시작하는 것을 선호할까요?

**<u>변수의 변화 폭을 비대칭 범위</u>**로 표현할 수 있기 때문입니다. [1, rows]를 범위로 사용하는 for문보다
[0, rows)를 범위로 사용하는 for문이 더 자연스럽습니다.